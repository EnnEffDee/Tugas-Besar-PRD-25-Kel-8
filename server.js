...
const server = http.createServer((req, res) => {
    ...
    if (req.method === "PATCH" && url.startsWith("/api/logs/toggle/")) {
        const timestamp = decodeURIComponent(
            url.replace("/api/logs/toggle/", ""),
        );
        ...
        try {
            ...
            const rows = lines.slice(1).map((line) => {
                ...
                const rowTs = cols[0]?.trim();
                const mode = cols[4]?.trim();
                let alasan = cols[5]?.trim();

                if (rowTs === timestamp) {
                    if (mode === "MASUK") {
                        alasan = alasan === "MASUK_GANDA" ? "-" : "MASUK_GANDA";
                    }

                    if (mode === "KELUAR") {
                        alasan =
                            alasan === "KELUAR_GANDA" ? "-" : "KELUAR_GANDA";
                    }
